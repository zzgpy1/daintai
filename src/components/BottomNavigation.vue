<template>
  <nav class="bottom-navigation block md:hidden fixed bottom-0 left-0 right-0 z-10 bg-white dark:bg-dark-surface border-t border-gray-200 dark:border-dark-gray">
    <div class="flex items-center justify-around py-2 px-2 h-[72px]">
      <router-link v-for="item in navigationItems" :key="item.name" :to="item.path" class="nav-item flex flex-col items-center justify-center py-1 px-2 rounded-ios transition-all active:scale-95 flex-1"
        :class="{
...
import { HomeIcon, SearchIcon, ClockIcon, HeartIcon } from '@heroicons/vue/24/outline'
...
const navigationItems = computed(() => [
  ...
  { name: 'Search', path: '/search', label: t('nav.search'), icon: SearchIcon },
